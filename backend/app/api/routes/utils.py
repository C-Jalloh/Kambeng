...
# Regenerate stored campaign QR URLs (page + direct)
@router.post("/qrcode/campaign/{slug}/regenerate")
async def regenerate_campaign_qr(slug: str, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    """Regenerate campaign QR codes and update stored URLs."""
    result = await db.execute(select(Campaign).where(Campaign.slug == slug))
    campaign = result.scalar_one_or_none()
    if not campaign:
        from fastapi import HTTPException
    ...
    # Only owner or admin may regenerate
    ...
    direct_url = f"{frontend_url}/quick-pay/{slug}"

    page_qr = generate_and_upload_qr(page_url, f"page_{slug}")
    direct_qr = generate_and_upload_qr(direct_url, f"direct_{slug}")

    campaign.qr_code_page_url = page_qr
    campaign.qr_code_direct_url = direct_qr
    db.add(campaign)
    await db.commit()
    await db.refresh(campaign)

    return {"slug": campaign.slug, "qr_code_page_url": campaign.qr_code_page_url, "qr_code_direct_url": campaign.qr_code_direct_url}
