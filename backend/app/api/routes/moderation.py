...
# --- Backwards-compatible fraud-report endpoints (tests expect /fraud-reports/*) ---
from app.models.fraud_report import FraudReport as _FraudReport
...
async def list_fraud_reports(
    ...
    result = await db.execute(select(_FraudReport).order_by(_FraudReport.created_at.asc()).offset(skip).limit(limit))
...
async def resolve_fraud_report(
    ...
    result = await db.execute(select(_FraudReport).where(_FraudReport.id == report_id))
    report = result.scalars().first()
    if not report:
        raise HTTPException(status_code=404, detail="Report not found")
    ...
    if action == "suspend_campaign" or action == "suspend_campaign":
        # suspend campaign
    ...
    report.resolved_by_admin_id = admin_user.id if hasattr(report, "resolved_by_admin_id") else None
    await db.commit()
    await db.refresh(report)
    return report
