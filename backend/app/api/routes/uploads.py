...
async def list_campaign_proofs(
    ...
    # If the viewer is not owner, admin, or donor, deny access to the proofs list
    if not (is_owner or is_admin or is_donor):
        raise HTTPException(status_code=403, detail="Not authorized to view proofs for this campaign")
...
async def upload_campaign_images(
    ...
    # Validate each uploaded file first (type, non-empty, size) before checking
    # campaign image count to ensure size errors are reported for oversized files.
    ...
    for upload, content in validated_files:
